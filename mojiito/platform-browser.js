 ...
 * License: MIT
 */
import { ApplicationRef, CORE_PROVIDERS, ComponentFactoryResolver, ComponentResolver, Injectable, InjectionToken, Injector, PlatformRef, ReflectiveInjector, RendererFactory, SkipSelf, createComponentFactory, createPlatformFactory, createView, resolveReflectiveProviders } from 'mojiito-core';
...
class CssSelector {
    ...
    static fromElement(element) {
        ...
        ListWrapper.forEach(element.classList, (c) => selector.addClassName(c));
        return selector;
    }
    /**
     * @param {?} selector
...
class SelectorMatcher {
     ...
     */
    match(cssSelector, matchedCallback = function () { }) {
...
class DomVisitor {
     ...
     * @return {?}
     */
    visitAttribute(element, attr, context) { }
    /**
     * @param {?} text
     * @param {?} context
     * @return {?}
...
class ParseError extends WrappedError {
    /**
     * @param {?} error
     */
    constructor(error) {
...
class MultipleComponentsOnElementError extends ParseError {
    /**
     * @param {?} components
     */
    constructor(components) {
 ...
 * @nocollapse
 */
ExpressionParser$$1.ctorParameters = () => [];
class Token$$1 {
    /**
     * @param {?} index
     * @param {?} type
...
class EventBindingParseResult extends BindingParseResult {
    /**
     * @param {?} eventName
     * @param {?} expression
     */
...
class Compiler {
     ...
     * @param {?} _bindParser
     */
    constructor(_resolver, _bindParser) {
    ...
    compileComponent(component) {
        ...
        const /** @type {?} */ viewDefinitionFactory = this._createComponentViewDef(component, metadata.providers, rendererType);
        // create a component factory for this component type
        const /** @type {?} */ componentFactory = createComponentFactory(metadata.selector, component, viewDefinitionFactory);
        compileSummary = {
            type: component,
     ...
     * @return {?}
     */
    _createProviderNodes(providers, nodes, nodeType) {
     ...
     * @return {?}
     */
    _createComponentRendererType(visitor) {
        return {
            visitor,
            data: null
        };
     ...
     * @return {?}
     */
    _createComponentViewDef(component, providers, componentRendererType) {
        const /** @type {?} */ viewDefinitionFactory = () => {
            const /** @type {?} */ nodes = [];
            let /** @type {?} */ nodeFlags = 16777216;
            // Create public provider instances and add to nodes
            let /** @type {?} */ publicProviders = {};
            if (providers) {
                this._createProviderNodes(providers, nodes, 128 /* TypeProvider */).forEach(node => {
                    publicProviders[node.provider.tokenKey] = node;
                });
                nodeFlags |= 128 /* TypeProvider */;
            }
            // Create component instance and add to nodes
            this._createProviderNodes([component], nodes, 16384 /* TypeComponent */);
...
class BrowserPlatformRef extends PlatformRef {
     ...
     * @param {?} _compiler
     */
    constructor(_injector, _resolver, _compiler) {
        super();
    ...
    bootstrapComponent(component) {
        ...
        const /** @type {?} */ app = (appInjector.get(ApplicationRef));
...
class DomTraverser {
     ...
     * @return {?}
     */
    traverse(node, visitor, context = null) {
        let /** @type {?} */ lclCntxt = context;
        this._nodeCount++;
...
class DefaultDomRenderer {
    ...
    listen(target, event, callback) {
        ...
        return () => target.removeEventListener(event, /** @type {?} */ (callback), false);
    }
}
class ParseableDomRenderer extends DefaultDomRenderer {
    /**
     * @param {?} _visitor
     * @param {?} hostElement
     */
    constructor(_visitor, hostElement) {
        super();
...
const platformBrowser = createPlatformFactory([PLATFORM_PROVIDERS, CORE_PROVIDERS]);
...
export { DOCUMENT, DomRendererFactory, BrowserPlatformRef, ExpressionParser$$1 as ExpressionParser, PLATFORM_PROVIDERS, platformBrowser, BindingParser as ɵb, Compiler as ɵa };
